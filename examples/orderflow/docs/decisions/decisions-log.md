...
**Charging is asynchronous — a queue between orders and payments** — checkout returns
to the shopper immediately rather than waiting on the provider; orders publishes
`charge-requested` onto the charge queue and payments charges the order off that queue.
Rejected: charging synchronously inside `POST /orders`, which turns a provider outage
into a checkout outage.

### Data Model

**One writer per table** — every table has exactly one component that writes it; every
other component goes through that owner's API or events — `orders` only by
services/orders, `charges` only by services/payments, `shipments` only by
services/fulfillment. Rejected: shared write access by convention alone, since nothing
enforces a convention and the first hotfix breaks it.

### Fulfillment Integration

**2025-11-04 — Payments publishes charge-settled; fulfillment consumes the event** —
payments publishes a `charge-settled` event once a charge settles, and fulfillment
consumes that event instead of reading `charges`. This supersedes the earlier approach in
which fulfillment polled the `charges` table on an interval, which coupled it to a schema
owned by another component; the event removes that coupling. Rejected: shortening the poll
interval, which reduces latency but leaves the coupling in place.
